...
def import_from_discovery(connector_id: str, session: Session) -> ImportAssetsResponse:
    ...
    """
    cached = connector_service.get_cached_discovery(connector_id, session)
    if not cached:
    ...
    if hasattr(cached, "assets") and cached.assets:
        for ad in cached.assets:
            p = ad.path
            meaningful = [x for x in p.lower().split("/") if x not in _SKIP_PREFIXES and len(x) > 2]
            aid = connector_id + "-" + "-".join(meaningful[-2:]) if len(meaningful) >= 2 else connector_id + "-" + _slugify(ad.name)
            existing_asset = session.get(Asset, aid)
            if not existing_asset:
                session.add(Asset(id=aid, name=ad.name, asset_type="equipment", description=f"Discovered from {connector_id} at {p}"))
                session.flush()
                assets_created += 1
            ...
            for var in ad.variables:
                ...
                if not existing_prop:
                    session.add(AssetProperty(
                        asset_id=aid, name=prop_name, display_name=var.name,
        ...
        for node in cached.nodes:
            parent_name = node.path.split("/")[-2] if "/" in node.path else "equipment"
            groups[parent_name].append(node)
            asset_names[parent_name] = parent_name
        ...
        for parent_name, nodes in groups.items():
            ...
            for node in nodes:
                prop_name = _slugify(node.name)
                existing_prop = session.exec(
                    select(AssetProperty).where(AssetProperty.asset_id == aid, AssetProperty.name == prop_name)
                ).first()
                if not existing_prop:
                    session.add(AssetProperty(
                        asset_id=aid, name=prop_name, display_name=node.name,
                        connector_id=connector_id, node_id=node.node_id,
                        writable=node.writable, unit=getattr(node, "unit", None),
